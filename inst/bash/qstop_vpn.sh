#!/bin/bash
#' ---
#' title: Stop VPN Connection
#' date:  2022-04-20 13:24:17
#' author: Peter von Rohr
#' ---
#' ## Purpose
#' Seamless stop command for vpn connection
#'
#' ## Description
#' Stop a given VPN connection
#'
#' ## Details
#' Wrapper around scutil to stop vpn connection
#'
#' ## Example
#' ./qstop_vpn.sh
#'
...
if [ $(scutil --nc list | grep $VPNNAME | grep '(Connected)' | wc -l) -gt 0 ]
then
  log_msg $SCRIPT " * Stopping VPN-Connection: $VPNNAME ..."
  scutil --nc stop $VPNNAME
else
  log_msg $SCRIPT " * $VPNNAME does not seam to be connected ..."
